    ...
    public class ContinuousCoordinateSystem : CoordinateSystemIfc
    {
        ...
        public LatLonZoom GetTranslationInLatLon(LatLonZoom oldCenterPosition, Point mouseMotion)
        {
            return new LatLonZoom(
                oldCenterPosition.lat + this.pixelsToDegrees((long)mouseMotion.Y, oldCenterPosition.zoom),
                oldCenterPosition.lon - this.pixelsToDegrees((long)mouseMotion.X, oldCenterPosition.zoom),
        ...
        {
            return new Point((int)this.degreesToPixels(point.lon - center.lon, center.zoom),
                (int)this.degreesToPixels(center.lat - point.lat, center.zoom));
        }
        ...
        {
            Point64 point = new Point64(this.degreesToPixels(llz.lon, llz.zoom),
                this.degreesToPixels(llz.lat, llz.zoom));
        ...
        public MapRectangle GetUnclippedMapWindow(LatLonZoom centerPosition, Size windowSize)
        {
            return new MapRectangle(
                this.GetTranslationInLatLon(centerPosition, new Point(windowSize.Width / 2, windowSize.Height / 2))
                    .latlon,
                this.GetTranslationInLatLon(centerPosition,
        ...
        public TileDisplayDescriptorArray GetTileArrayDescriptor(LatLonZoom center, Size windowSize)
        {
            ...
            tileDisplayDescriptorArray.topLeftTile =
                this.GetTileContainingLatLonZoom(new LatLonZoom(unclippedMapWindow.GetNW(), center.zoom));
            TileAddress tileContainingLatLonZoom =
                this.GetTileContainingLatLonZoom(new LatLonZoom(unclippedMapWindow.GetSE(), center.zoom));
            ...
            Point64 point = new Point64(this.degreesToPixels(unclippedMapWindow.GetNW().lon, center.zoom),
                this.degreesToPixels(unclippedMapWindow.GetNW().lat, center.zoom));
