...
namespace BigMapMaker
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            RectLatLng area = RectLatLng.FromLTRB(25.013809204101563,
                54.832138557519563,
                25.506134033203125,
                54.615623046071839);
            if (!area.IsEmpty)
            {
                try
                {
                    ...
                    int padding = 22;
                    {
                        using (Bitmap bmpDestination =
                            new Bitmap((int)(pxDelta.X + padding * 2), (int)(pxDelta.Y + padding * 2)))
                        {
                                ...
                                foreach (var p in tileArea)
                                {
                                    ...
                                    foreach (var tp in provider.Overlays)
                                    {
                                        ...
                                        if (tile != null)
                                        {
                                            using (tile)
                                            {
                                                long x = p.X * provider.Projection.TileSize.Width - topLeftPx.X +
                                                         padding;
                                                long y = p.Y * provider.Projection.TileSize.Width - topLeftPx.Y +
                                                         padding;
                                                {
                                                    gfx.DrawImage(tile.Img,
                                                        ...
                                                        provider.Projection.TileSize.Height);
                                                }
                                            }
                                        }
                                    }
                            ...
                            // draw info
                            {
                                ...
                                using (Graphics gfx = Graphics.FromImage(bmpDestination))
                                {
                                    ...
                                    using (Pen p = new Pen(Brushes.Red, 3))
                                    {
                                        ...
                                        gfx.DrawString(rightBottom,
                                            f,
                                            p.Brush,
                                            rect.Right - s2.Width - s2.Height / 2,
                                            rect.Bottom - s2.Height - s2.Height / 2);
                                    }
                                    ...
                                    using (Pen p = new Pen(Brushes.Blue, 1))
                                    {
                                        ...
                                        gfx.DrawString(leftBottom,
                                            f,
                                            p.Brush,
                                            rect.X + 10,
                                            rect.Bottom - s.Height - 20);
                                    }
                                }
                            }
